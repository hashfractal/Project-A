...
public class Player : Singleton<Player>
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        else if(collision.tag == "Room")
        {
            ...
            for (int i = 0; i < RoomController.Instance.loadedRooms.Count; i++)
            {
                if (RoomController.Instance.currRoom.parent_Position == RoomController.Instance.loadedRooms[i].parent_Position)
                {
                    RoomController.Instance.loadedRooms[i].gameObject.SetActive(true);
                }
                else
                {
                    RoomController.Instance.loadedRooms[i].gameObject.SetActive(true);
